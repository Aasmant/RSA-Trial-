...
public class ApiController {
    ...
    public ResponseEntity<?> register(@RequestBody Map<String, String> data) {
        ...
        try {
            ...
            // Simple hash (In real app use BCrypt, Python used pbkdf2:sha256)
            // For similarity with Python's 'generate_password_hash', let's stick to a
            // simple hashing or mimics behavior
            // Let's use a very basic SHA-256 for simplicity as dependency management for
            // pbkdf2 is complex manually without spring security
            // Or better, just store it (Very bad, but strictly speaking we want to match
            // Python behavior which hashed it).
            // Python used: generate_password_hash(password, method='pbkdf2:sha256')
            // Detailed replication is optional, but security best practice is BCrypt.
            // Let's assume we use a placeholder or simple hash.
            String passwordHash = DigestUtils.md5DigestAsHex(password.getBytes()); // Intentionally weak to reflect
                                                                                   // "Educational Purpose" or just
                                                                                   // simplicity
    ...
    public ResponseEntity<?> login(@RequestBody Map<String, String> data) {
        ...
        // Verify hash (Using MD5 for simplicity in this port as placeholder for PBKDF2)
    ...
    public ResponseEntity<?> uploadFile(@RequestHeader("Authorization") String token,
        ...
        try {
            ...
            // VULNERABILITY 9: No file size validation (handled by high limit in
            ...
            // File Hash
            String fileHash = DigestUtils.md5DigestAsHex(fileData); // Using MD5 as simple hash, python used sha256. Not
                                                                    // critical.
    ...
    public ResponseEntity<?> decryptFile(@RequestHeader("Authorization") String token, @PathVariable Long fileId,
        ...
        try {
            ...
            if (!privateKeyPem.contains("\n") && privateKeyPem.contains("-----BEGIN")) {
                // Simplified reformatting logic or assume client sends correct format
            }
